    ...
    <div className="w-72 glass border-l border-border/50 flex flex-col">
      ...
      {selectedTrip && (
        <div className="p-3 border-t border-border/50 bg-secondary/20">
          <p className="text-xs font-medium text-foreground mb-2">Selected Trip Details</p>
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="bg-background/50 rounded p-2">
              <p className="text-muted-foreground">Distance</p>
              <p className="font-mono font-medium text-foreground">{selectedTrip.total_distance} km</p>
            </div>
            <div className="bg-background/50 rounded p-2">
              <p className="text-muted-foreground">Duration</p>
              <p className="font-mono font-medium text-foreground">{selectedTrip.duration_minutes} min</p>
            </div>
            <div className="bg-background/50 rounded p-2">
              <p className="text-muted-foreground">Max Speed</p>
              <p className="font-mono font-medium text-foreground">{selectedTrip.max_speed} km/h</p>
            </div>
            <div className="bg-background/50 rounded p-2">
              <p className="text-muted-foreground">Avg Speed</p>
              <p className="font-mono font-medium text-foreground">{selectedTrip.avg_speed} km/h</p>
            </div>
          </div>
          <p className="text-[10px] text-muted-foreground mt-2">
            {selectedTrip.coordinates.length} coordinates recorded
          </p>
        </div>
      )}
    </div>
    ...
    >
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>{formatTime(trip.start_time)} - {formatTime(trip.end_time)}</span>
        </div>
